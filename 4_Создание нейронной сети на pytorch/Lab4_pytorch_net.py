...
pred_lbl = labels[pred.max(1).indices.numpy()] # предсказанные названия классов
true_lbl = labels[y.max(1).indices.numpy()]    # истинные названия классов
...
X = torch.Tensor(df.iloc[:, [0:8]].values) # выделяем признаки (независимые переменные)
